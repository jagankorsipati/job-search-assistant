...
Phase 6 is released as `v0.6.0-truthful-tailoring`. Ownership applies across proposal CRUD, both reviews and approvals, and ephemeral download. Approval history remains owned and retained; draft deletion cannot erase it. No new persisted export resource or shared lookup exists. Source replacement keeps its row identity but changes version/checksum; existing proposals never silently retarget. Local rendering covers the current supported DOCX boundary; target Word fidelity remains separate evidence. See [Phase 6 evidence](security/phase-6-verification.md).

## Sources of truth

Phase 7A adds no owned table or shared lookup. Documents loads one owned proposal/source and only explicitly selected current owner-confirmed facts through owner-scoped repository reads. Foreign, missing, draft and archived references produce equivalent safe failures, including for ADMIN. The private immutable request context retains owner, proposal/source revisions and alias-to-fact/version mappings; the provider receives only local aliases and selected content in a dedicated DTO. A response has no authority to change proposals, facts, approval or export. Future import requires owner-scoped transactional freshness revalidation. Phase 7B adds optional OpenAI/Claude transport without changing this ownership boundary. Provider/model selection belongs to administrator/operator process configuration, never to end users, and grants no ADMIN access to another owner's content. There is no public drafting caller or live user-data transmission; selected text is still personal data, not anonymized. See [ADR-020](decisions/ADR-020-optional-grounded-drafting-contracts.md) and [ADR-021](decisions/ADR-021-opt-in-grounded-drafting-adapter.md).

Phase 6E resolved-target review, attestation, and download use the existing actor and owner-scoped Documents repositories. Administrators cannot export another member's proposal. V13 adds a target binding to approval history without transferring ownership or retrofitting old approvals. Generated bytes are ephemeral; they never replace the owned source resume or introduce a shared export lookup. The final locked version/decision check authorizes release. See [ADR-019](decisions/ADR-019-controlled-single-proposal-docx-download.md).
