...
Phase 7A adds no owned table or shared lookup. Documents loads one owned proposal/source and only explicitly selected current owner-confirmed facts through owner-scoped repository reads. Foreign, missing, draft and archived references produce equivalent safe failures, including for ADMIN. The private immutable request context retains owner, proposal/source revisions and alias-to-fact/version mappings; the provider receives only local aliases and selected content in a dedicated DTO. A response has no authority to change proposals, facts, approval or export. Future import requires owner-scoped transactional freshness revalidation. No live AI drafting or external transmission exists; selected text is still personal data, not anonymized. See [ADR-020](decisions/ADR-020-optional-grounded-drafting-contracts.md).
