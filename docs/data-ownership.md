...
- Base resume responses do not return `owner_account_id`, storage keys, or filesystem paths. Phase 6C includes the SHA-256 checksum in owner-scoped metadata so proposal creation can pin exact content. Download is owner-scoped and attachment-only.
...
Phase 6C adds a Profile-integrated frontend using these same owner-scoped APIs. Explicit request projection excludes owner/account fields and storage attributes. Confirmed-fact selection never imports fit links or infers support. The browser preserves decision-history refusal and sends expected versions; it cannot override source attribution, eligibility, or ownership. See [tailoring workflow](tailoring-workflow.md).
