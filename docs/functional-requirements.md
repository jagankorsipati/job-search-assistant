...
- **FR-070:** Phase 6B exposes authenticated no-store owner-scoped r̩sum̩ tailoring proposal CRUD APIs that derive ownership server-side, omit owner identifiers and storage paths, use bounded lists, require CSRF for mutation, and provide no administrator cross-user bypass.
- **FR-071:** Phase 6B review responses return proposed text, user-supplied original text labeled as not checked against the document, target reference, pinned source r̩sum̩ attribution, current evidence fact IDs/versions, eligibility, and a review revision binding the exact reviewed state.
- **FR-072:** Phase 6B approval requires expected proposal version, exact reviewed revision, and explicit owner attestation that the wording accurately represents their experience without unsupported claims; approval revalidates and locks the source r̩sum̩ and supporting facts transactionally, and changed source, changed proposal, missing evidence, unconfirmed/archived/deleted/foreign evidence, or changed fact versions block approval.
- **FR-073:** Phase 6B records append-only approval/rejection history without erasing it through draft deletion, treats later proposal/source/evidence changes as stale on evaluation, and does not make approval an export-ready document, document edit, application submission, AI output, or proof that linked facts establish every proposed claim.
