...
Phase 5 is complete. Fit analysis uses manually confirmed requirements and explicit evidence links, separating support from review coverage. Phase 6A-C are implemented; bounded single-proposal DOCX export is implemented through V13, but Phase 6 remains open for rendering and release gates. Tailoring supports manual drafts, owner-attested wording review, separate actual-target review and approval, and deliberate ephemeral download. No automatic fact extraction, suggestions, AI drafting, scraping, cover-letter generation, or application submission is implemented. See the [Phase 6 verification record](docs/security/phase-6-verification.md) for evidence and outstanding gates.
...
The final identity threat model and evidence are recorded in the [Phase 2 verification matrix](docs/security/phase-2-verification.md). Candidate-profile and base-resume verification evidence is recorded in the [Phase 3 verification matrix](docs/security/phase-3-verification.md). Job/application verification evidence is recorded in the [Phase 4 verification matrix](docs/security/phase-4-verification.md). Fit-analysis security and release evidence is recorded in the [Phase 5 verification matrix](docs/security/phase-5-verification.md). Future household deployment must pass the separate [deployment security checklist](docs/security/deployment-checklist.md).
...
Base résumé API endpoints live under `/api/documents/base-resume`. Each owner can store one current PDF or DOCX up to 5 MiB. Uploading stores the source document only; it does not parse, extract, confirm, import, invoke AI, or change career facts. `POST` creates the initial document, `PUT` replaces it with `expectedVersion`, and `/download` streams it as an attachment. Owner-scoped metadata includes `sha256Checksum` for exact tailoring source attribution but never exposes owner IDs, storage keys, or filesystem paths. Configure storage with `BASE_RESUME_STORAGE_ROOT`; the local default is for development only and must be treated as sensitive data.

Resume tailoring proposal API endpoints live under `/api/documents/resume-tailoring-proposals`. They expose owner-scoped draft CRUD, read-only review, explicit approval, and rejection for manually authored proposal text only. Review responses label original text as user-supplied and not checked against the source document, bind the proposal/source/evidence state with a review revision, and explain that linked confirmed facts are support references rather than proof of every proposed claim. Approval requires CSRF, expected version, the exact reviewed revision, and explicit owner attestation; it revalidates the source resume and supporting fact versions transactionally. Approval and rejection are history records only, not export readiness, document editing, application submission, AI output, or external action.
...
Phase 5B adds an internal deterministic scoring policy, `DETERMINISTIC_FIT_V1`. Only confirmed requirements participate; draft and rejected requirements are excluded from denominators but counted in the result. Required requirements have weight 2, preferred and unspecified requirements have weight 1. Demonstrated evidence receives credit 1.0, partial support receives 0.5, and not-demonstrated, contradicted, conflicting, and unassessed requirements receive 0.0. Support and coverage scores use exact decimal arithmetic and half-up whole-number rounding. Contradictions, conflicting evidence, gaps, partial gaps, and per-importance breakdowns are returned as structured reason codes. Results are computed on demand and never persisted.

The fit review workspace is reached from a description snapshot's `Review fit` action and can be refreshed at `/jobs/{jobId}/snapshots/{snapshotId}/fit`. The path contains only job and snapshot identifiers; requirement text, source excerpts, evidence notes, descriptions, owner IDs, account IDs, filters, and analysis payloads are not stored in URLs or browser storage. Requirement creation defaults to `DRAFT`, and confirmation, rejection, return-to-draft, deletion, evidence linking, relationship changes, and link removal are deliberate user actions backed by the Phase 5A APIs. Evidence choices are loaded only from existing owner-scoped profile, confirmed career-fact, and base-resume metadata APIs. The browser does not calculate scores; it displays the Phase 5C response, including non-scorable status, support, coverage, breakdowns, explanations, gaps, partial evidence, contradictions, and conflicts.
...
The job, fit-review, and application frontend workspaces are available at `/jobs`, `/jobs/{jobId}/snapshots/{snapshotId}/fit`, and `/applications` after sign-in through the lightweight in-app navigation. Refreshing those paths restores the existing session and reopens the workspace; unauthenticated or expired sessions return to login. The browser keeps job, snapshot, requirement, evidence-link, analysis, application, and status-history data only in memory and never submits owner/account identifiers. Job posting URLs are displayed as references only; the frontend does not fetch, scrape, or submit to them.
...
# Full-stack browser identity, profile, job/application, and fit-analysis security suite; Docker and installed Chromium are required
npx.cmd playwright install chromium
cd ..
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-browser-e2e.ps1
```

GitHub Actions repeats these checks in parallel backend, frontend, PostgreSQL Compose smoke, and browser E2E jobs. The browser job uses a disposable named database volume, restarts PostgreSQL to check retained state, and deletes that test volume in cleanup. Process-only test credentials and independent identity, profile, job/application, fit-analysis and tailoring specs are used. Screenshots, traces, videos, browser profiles, raw error contexts and HTML reports are disabled or discarded. Only sanitized startup/readiness status and test locations may be retained, not assertion values or document contents. All Actions are pinned to immutable commit SHAs. Run `scripts/run-browser-e2e.ps1 -VerificationMode Matrix` for individual normal/reverse runs followed by three consecutive full-suite passes.

## Current tailoring implementation and open gates

Controlled single-proposal DOCX download is implemented in the tailoring workspace. `GET /api/documents/resume-tailoring-proposals/{id}/resolved-review?expectedVersion=...` returns actual source text, intended replacement, location/source attribution, evidence versions, and `resolvedRevision`. `POST .../approve-resolved` requires `expectedVersion`, that revision, and explicit `attestedTargetAndExperience: true`. A separate `POST .../export` with expected version/revision returns a validated `tailored-resume.docx` attachment after final transactional checks. Legacy approval is insufficient; unsupported or stale targets refuse. No original file is overwritten or generated file persisted. See [ADR-019](docs/decisions/ADR-019-controlled-single-proposal-docx-download.md). Rendering verification remains incomplete, so Phase 6E is not marked complete; Phase 6F browser release verification remains pending.

The authenticated `/profile/tailoring` workspace opens from Profile beside the base resume. Users manually create and edit proposals, select confirmed career facts, load a before/after review, and explicitly attest before approval. See [tailoring workflow](docs/tailoring-workflow.md) for conflict recovery, history, and eligibility limits. Controlled export is implemented, not release-certified: Phase 6 remains open for rendered-layout evidence and final local/hosted release verification. Phase 7 has not started.
