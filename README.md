...
Phase 5E is in verification: authenticated users can open `/jobs/{jobId}/snapshots/{snapshotId}/fit` from the Jobs workspace, manually review requirements for one immutable snapshot, explicitly link eligible candidate evidence, and view the server's explainable fit analysis. The UI presents evidence support and review coverage separately, handles no-confirmed-requirement snapshots without misleading percentages, preserves unsaved work on optimistic conflicts, and keeps fit data only in React memory. The real-browser fit-analysis security specification exercises the full workflow against React, Spring Boot, PostgreSQL, Spring Session, CSRF, Flyway V10, and `DETERMINISTIC_FIT_V1`. Persisted scores, AI analysis, automatic extraction, automatic evidence matching, scraping, resume tailoring, reminders, application submission, and Phase 6 work are not included.
...
GitHub Actions repeats these checks in parallel backend, frontend, PostgreSQL Compose smoke, and browser E2E jobs. The browser job uses a tmpfs-backed disposable database and process-only test credentials and discovers the identity, profile, job/application, and fit-analysis security specs. Playwright screenshots, traces, videos, browser profiles, raw error contexts, and HTML reports are disabled or discarded because failure artifacts could retain invitation fragments, cookies, credentials, requirements, evidence notes, descriptions, or resume metadata. On failure, CI retains only sanitized text containing process/readiness state, filtered startup errors, PostgreSQL health and active-administrator count, and safe Playwright status/locator context. All Actions are pinned to immutable commit SHAs.
